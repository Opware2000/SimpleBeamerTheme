\documentclass[xetex, aspectratio=169,professionalfont]{beamer}
...
%% Title and authors
\title{\fontbold A Non-intrusive Beamer Theme Focusing on Content}
...
\begin{frame}
	...
	\ribbon{AlertColor}{\fontbold \Huge A New Theme}
\end{frame}
...
\begin{frame}
	\vspace{2\baselineskip}
	\ribbon{MyGreen}{\fontbold \Huge Tools}
\end{frame}
...
\begin{frame}[fragile]{Colorful side headings}
	...
	\fancybox[MyPink]{Some Important Points}{
		So here we go:
		\begin{enumerate}
			\item First point
			\item Second point
...
\begin{frame}
	\mainresult{Our Main Result}{
		Sometimes we want to dedicate one slide to show our result. This whole slide just shows our result, so that viewers can see it for a while and soak it. It should look beautiful.
		}
\end{frame}
...
\begin{frame}
\vspace{2\baselineskip}
\ribbon{MyOrange}{\fontbold \Huge How Does It Work?}
\end{frame}
...
\fancybox[MyBlue]{What about Fonts?}{
	I have provided all the used fonts in Fonts directory. As far as I know these are open-source fonts.
}
...
\begin{frame}
	\vspace{2\baselineskip}
	\ribbon{DarkGold}{\fontbold \Huge What Next?}
\end{frame}
